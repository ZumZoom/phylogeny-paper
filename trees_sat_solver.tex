...
Algorithm consits of three stages

\begin{enumerate}
\item Preprocessing
\item Encoding boolean formula
\item Solving boolean formula
\item Postprocessing
\end{enumerate}
...
After we split out task into a set of less complex tasks, we add a dummy root to each tree of each task along with the new dummy leaf for tree consistency. This is done to ensure that all the trees in the input share common root. This dummy root will be deleted on the postprocessing stage.
...
The input is several rooted binary phylogenetic trees
    
$V, R, L$

\begin{itemize}
\item $left_{v,u}, right_{v,u} \forall v \in V, u \in PC(v)$ true iff node $v$ has node $u$ as left (right) child.

\item $parent_{v,u} \forall v \in L \cup V / root, u \in PP(v)$ true iff node $v$ has node $u$ as parent.

\item $lp_{v,u}, rp_{v,u} \forall v \in R, u \in PC(v)$

\item $ch_{v,u} \forall v \in R, u \in PP(v)$
...
Constraints
\begin{itemize}
\item at-most-one and at-least-one clauses for variables $left$, $right$, $parent$, $lp$, $rp$, $ch$, $up$, $x$
\item left child must have number less then the right one. This constraint can be translated as 
      $(\neg left_{v,u} \vee \neg right_{v,w}) \forall u, w: u \geq w$
\item same constraints for variables $lp_{v,u}$ and $rp_{v,u}$
\item constraints to connect parents with children:
    \begin{itemize}
    \item ($\neg left_{v,u} \vee parent_{u,v}$)
    \item ($\neg right_{v,u} \vee parent_{u,v}$)
    \item ($\neg parent_{u,v} \vee left_{v,u} \vee right_{v,u}$)
    \end{itemize}
\item constraints to connect parents with reticulation children
    \begin{itemize}
    ...
    \end{itemize}
\item constraints to connect reticulation parents with children
    \begin{itemize}
    \item ($\neg ch_{v,u} \vee parent_{u,v}$)
    \item ($\neg parent_{u,v} \vee ch_{v,u}$)
    \end{itemize}
\item constraints to connect reticulation parents with reticulation children
    \begin{itemize}
    \item ($\neg ch_{v,u} \vee lp_{u,v} \vee rp_{u,v}$)
    \item ($\neg lp_{u,v} \vee ch_{v,u}$)
    \item ($\neg rp_{u,v} \vee ch_{v,u}$)
    \end{itemize}
\item constraints to order child and parents of reticulation node. $\forall v, u, w: v \in R, u \in PC(v), u \in V, w \in V, w \leq u$:
    \begin{itemize}
    \item ($\neg ch_{v,u} \vee \neg lp_{v,w}$)
    \item ($\neg ch_{v,u} \vee \neg rp_{v,w}$)
    \end{itemize}
...
There were network definition constraints.
