...
\begin{abstract}
  ...
  the problem to an instance of the Boolean satisfiability problem. The main idea of our algorithm is to look over
  possible hybridosation numbers and to constrcut for each of them a Boolean formula that is satisfiable iff there
  exists a network with such hybridisation number. We also created a software tool called PhyloSAT which implements our algorithm.
...
\begin{figure}[t]
  ...
  \begin{minipage}[b]{0.59\linewidth}
    ...
    \centering{\\ (a) Three gene trees over the set of taxa \\ \{a, b, c, d, e\}}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.40\linewidth}
    \includegraphics[width=1.3in]{pics/ans.eps}
    \centering{\\ (b) A hybridisation network for the trees from Fig.~\ref{network-example}~(a)}
  \end{minipage}
...
network with this hybridization number. We look through possible values of $k$ starting from the highest 
...
that the node can have at least one parent and at most one parent. For example for parents of regular node $v$ this can be expressed 
...
They are listed in the Section 5 of Table~\ref{network-table}.
...
relative numbers of children and parents of reticulation nodes. They are defined in the Section 10 of Table~\ref{network-table}.
...
mapping of network nodes to tree nodes. An example of such mapping is illustrated in Figure~\ref{mapping-example}.
...
most one node from network corresponds to the tree node. These clauses are defined in sections 1-2 of Table~\ref{mapping-table}.
...
consider an obvious observation that $x_{t,v_t,v}$ implies $u_{t,v}$. See the Section 3 of Table~\ref{mapping-table} for these clauses.
...
Furthermore if we know that node $v$ is leaf and its parent $u$ corresponds to node $u_t$ from tree $t$, 
then we can conclude that $a_{t,v,u}$ is true i.e. $u$ is the direct parent of $v$ in tree $t$. Same observation 
can be done for non-leaves, i.e. if we know that $v$ corresponds to $v_t$ and other node $u$ corresponds to $u_t$ 
and $u_t$ is parent of $v_t$ then $u$ is direct parent of $v$ considering tree $t$. On the other hand if we know 
that $v$ corresponds to $v_t$ and we know that $u$ is direct parent of $v$ considering tree $t$, then $u$ should 
correspond to parent of $v_t$. We also should take care about numeration so we add constraint that says that if node 
...
presented in the Section 4 of Table~\ref{mapping-table}.

We also add some heuristic constraints related to the trees structure. Notice that the number of the node in the network 
can not be less than the  size of the sub-tree of corresponding node $v_t$ in tree $t$ and also it can not be greater than 
size of the tree minus depth of $v_t$. Also if node $v_t$ in tree $t$ and node $v_{t'}$ in tree $t'$ have disjoint sets of 
taxa in their subtrees then they can not be mapped to the same node in network. These clauses can be found in the Section 5 of 
...
And vice versa if we know that $u$ is parent of $v$ in tree $t$ then 
...
These clauses are listed in the Section 1 of Table~\ref{child-parent-table}.
...
Now consider the reticulation node $v$ and its parent $u$. If $u$ is reticulation node then we should share information 
...
only when direction of $u$ matches direction specified in $v$. These constraints are presented in the Section 2 of Table~\ref{child-parent-table}.
...
for these clauses. If $u$ is reticulation node and its direction matches direction specified in $v$ and $v$ is used then $u$ should also be used.
...
if the child of node $v$ is regular node then we should use $v$ for displaying. 
...
exist node $w$ that its number is less than number of $v$ and $a_{t,u,w}$ is true. And for all $w$ with numbers 
greater than $v$ we add clauses to share information of $a$ variables. See the Section 5 of Table~\ref{child-parent-table} for these clauses.
...
processor on Ubuntu 14.04. All tests were run with the time limit of 1000 seconds. For comparison we also ran PIRN$\mathrm{_C}$ and PIRN$\mathrm{_{CH}}$ 
on the same test cases. 

9 out of 57 test cases were not solved even by heuristic algorithms in time. PhyloSAT was able to produce optimal answer for 28
...
On all the test cases running time of PIRN$\mathrm{_C}$ was worse than running time of PhyloSAT.
Even PIRN$\mathrm{_{CH}}$ did not solve 2 of these 28 test cases in time and its running time was better than running time of PhyloSAT only
on 5 test cases and significantly worse on 2 test cases. Twelve more test cases were not solved by PIRN$\mathrm{_C}$ but PhyloSAT
...
We have proposed an algorithm for construction an exact parsimonious hybridization network from multiply trees.
An implementation of our algorithm is available for download at \url{https://github.com/ctlab/PhyloSAT}.
Experiments have shown that PhyloSAT outperforms PIRN$\mathrm{_C}$ in all cases and performs reasonably well comparing 
to the heuristic PIRN$\mathrm{_{CH}}$. 
However in the cases of high hybridization numbers search and construction of optimal network is still a very challenging problem. 
In future we plan to use existing heuristics and estimations on lower and upper bounds of hybridization number to 
limit searching bounds and thus reduce the running time of our algorithm.
